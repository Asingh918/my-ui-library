...
describe('Table Component', () => {
  ...
  test('table has grey background-color when disabled', () => {
    ...
    expect(container.querySelector('table')).toHaveStyleRule('background-color', '#0d0d14');
  });

  test('table header has grey background-color when disabled', () => {
    const { container } = renderTable(true);
    expect(container.querySelector('thead')).toHaveStyleRule('background-color', '#1e293b');
  });
});
